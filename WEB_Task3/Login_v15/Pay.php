...
foreach($cart->getcid() as $i => $item_id){

$select = $DBConnect->query("select * from tbl_Item WHERE Item_ID ='".$item_id."'");
$row = $select->fetch_assoc();
	
if($cart->getcquan()[$i] >0){
?>
...
<!--https://www.google.com/search?q=Happy+bithday+21th&tbm=isch&ved=2ahUKEwj2w9q83Z7oAhVOiRoKHUQ1CLIQ2-cCegQIABAA&oq=Happy+bithday+21th&gs_l=img.3...23463.23999..24606...0.0..0.392.1252.2-2j2......0....1..gws-wiz-img.......0i10i24.8vrpOm5cA80&ei=c01vXra-Os6SasTqoJAL&bih=935&biw=927&rlz=1C1GCEU_enZA887ZA887&hl=en#imgrc=LpQPKNkOCvjzSM-->	
		...
		<td><?php echo $cart->getcquan()[$i];?></td>
...
$Quantity = $Quantity + $cart->getcquan()[$i];
$Price = $Price + ($row["Sell_Price"] * $cart->getcquan()[$i]);
}
 ?>
 <tr>
	<td colspan= '2'>Totals </td>
	<td>Total:<?php echo $Price ?>  </td>
	<form method="POST" action="Pay.php">
...
include_once("DBConn.php");
	if(isset($_POST['Pay'])){
		...
		if(isset($_SESSION['userid'])){
			...
			//problem starts here
			$sqlInsP = "Insert into tbl_order (Total_Qauntity,Total_Price,tbl_Customer_Customer_ID) values ('$Quantity','$Price','$id')";
			
			if($DBConnect->query($sqlInsP)){
			...
			echo("Error description: " . $DBConnect -> error);
				...
				$sqlInsP2 = "Insert into tbl_invoice (tbl_Order_Order_ID,tbl_Item_Item_ID,Total_Price,Total_Quantity) values ('$lastid','$item_id','$Price','$Quantity')";
				
				$QresultA2 = $DBConnect->query($sqlInsP2);
			
				}
			
		
		//order Created
		echo "<script> alert('Order Placed');window.location='ShopPage.php'</script>";
		$cart->emptycart();
